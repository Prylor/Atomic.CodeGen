...
public static class RenameCommand
{
	public static Command Create()
	{
		...
		{
			await ExecuteAsync(ctx, projectOption, typeOption, apiOption, nameOption, toOption, dryRunOption, renameFileOption, verboseOption, yesOption);
		});
		return command;
	}
	...
	private static async Task ExecuteAsync(
		...
		Option<string?> apiOption,
		Option<string?> nameOption,
		Option<string?> toOption,
		Option<bool> dryRunOption,
		Option<bool> renameFileOption,
		Option<bool> verboseOption,
		Option<bool> yesOption)
	{
		string? projectPath = ctx.ParseResult.GetValueForOption(projectOption);
		...
		if (isInteractive)
		{
			...
			IReadOnlyCollection<ApiEntry> apiEntries = await ConsoleUI.WithSpinnerAsync("Loading EntityAPIs...", async () => await orchestrator.GetAllApisAsync());
			...
			{
				ApiEntry apiEntry = ConsoleUI.SelectApi(apiEntries);
				if (apiEntry == null)
				{
					return;
				}
			...
			{
				(ApiEntry, string)? tuple = ConsoleUI.SelectBehaviour(apiEntries);
				if (!tuple.HasValue)
				{
					return;
				}
				renameType = RenameType.Behaviour;
				var (behaviourApi, behaviourName) = tuple.Value;
				oldName = behaviourName;
				ownerName = behaviourApi.ClassName;
				break;
			}
			case ConsoleUI.RenameCategory.Domain:
			{
				IReadOnlyCollection<DomainEntry> domainEntries = await ConsoleUI.WithSpinnerAsync("Loading EntityDomains...", async () => await orchestrator.GetAllDomainsAsync());
				if (domainEntries.Count == 0)
				{
					Logger.LogError("No EntityDomains found in project");
					return;
				}
				DomainEntry domainEntry = ConsoleUI.SelectDomain(domainEntries);
				if (domainEntry == null)
				{
					return;
				}
		...
		if (!yes && !ConsoleUI.ConfirmRename(context))
		{
			Logger.LogInfo("Rename cancelled");
			return;
		}

		if (!ConsoleUI.WithSpinner("Applying changes...", () => orchestrator.Execute(context)))
		{
